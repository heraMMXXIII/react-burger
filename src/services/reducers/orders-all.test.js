...
describe("orders-all reducer", () => {
  ...
  it("should handle ORDERS_ALL_SUCCESS", () => {
    expect(
      ordersAllReducer(initialState, { type: ORDERS_ALL_SUCCESS })
  ...
  it("should handle ORDERS_ALL_CLOSED", () => {
    expect(ordersAllReducer(initialState, { type: ORDERS_ALL_CLOSED })).toEqual(
      { ...initialState, error: null, connected: false }
    );
  });
